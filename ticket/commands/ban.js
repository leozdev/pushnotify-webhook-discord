...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (!executer.permissions.has(client.discord.Permissions.FLAGS.BAN_MEMBERS)) return interaction.reply({
      content: 'Você não tem permissão para este comando! (`BAN_MEMBERS`)',
      ephemeral: true
    });

    if (user.roles.highest.rawPosition > executer.roles.highest.rawPosition) return interaction.reply({
      content: 'Você não pode banir este membro',
      ephemeral: true
    });
    ...
    if (interaction.options.getString('raison')) {
      ...
      interaction.reply({
        content: `**${user.user.tag}** Foi Banido!`
      });
    } else {
      user.ban({
        days: 1
